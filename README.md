...
## 它做什么
- 常驻桌面显示多个平等卡片，单卡和卡组都能存在
- 每张卡片里的倒计时都可以设置名字和目标日期
- 双击标题或日期牌即可打开“编辑倒计时”
- 点击页迹切换，拖出页迹可把某一页拆成新卡片
- 拖动整张卡片窗口到另一张卡片上，可合并成卡组
- 长按或右键页迹可打开“倒计时总览”
- 右键卡片可打开“偏好设置”，目前提供开机启动、透明度和整理动作
- 倒计时总览支持切换、找回、定位和重置所有卡片位置
- 每天稳定随机一句副标题
- 点摘录卡右侧的花朵，今天可以再抽一句
- 启动时会自动把旧的 `countdown_pages.json` 迁移到新的工作区文件
- 自动把默认配置和内容库复制到用户目录

## 目录
- `App/`：SwiftUI + AppKit 源码和资源
- `preview/`：不依赖 macOS 的网页预览控制台
